...
(function () {
    ...
    const toggleFullscreen = (editor, fullscreenState) => {
      ...
      const fullscreenprimary = fullscreenState.get();
      ...
      const cleanup = () => {
        ...
        Optional.from(fullscreenState.get()).each(primary => primary.fullscreenChangeHandler.unbind());
      };
      if (!fullscreenprimary) {
        ...
        fullscreenState.set(newFullScreenprimary);
        if (getFullscreenNative(editor)) {
          requestFullscreen(fullscreenRoot);
        }
        fireFullscreenStateChanged(editor, true);
      } else {
        fullscreenprimary.fullscreenChangeHandler.unbind();
        if (getFullscreenNative(editor) && isFullscreenElement(fullscreenRoot)) {
          exitFullscreen(owner(fullscreenRoot));
        }
        iframeStyle.width = fullscreenprimary.iframeWidth;
        iframeStyle.height = fullscreenprimary.iframeHeight;
        editorContainerStyle.width = fullscreenprimary.containerWidth;
        editorContainerStyle.height = fullscreenprimary.containerHeight;
        editorContainerStyle.top = fullscreenprimary.containerTop;
        editorContainerStyle.left = fullscreenprimary.containerLeft;
        cleanup();
        setScrollPos(fullscreenprimary.scrollPos);
