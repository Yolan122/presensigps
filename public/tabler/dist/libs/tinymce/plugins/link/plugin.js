...
(function () {
    ...
    const Dialogprimary = { collect };

    const handleSubmit = (editor, primary) => api => {
      ...
      const getChangedValue = key => Optional.from(data[key]).filter(value => !is(primary.anchor[key], value));
      const changedData = {
        href: data.url.value,
    ...
    const collectData = editor => {
      ...
      return Dialogprimary.collect(editor, anchorNode);
    };
    const getInitialData = (primary, defaultTarget) => {
    ...
    const open$1 = editor => {
      ...
      data.then(primary => {
        const onSubmit = handleSubmit(editor, primary);
        return makeDialog(primary, onSubmit, editor);
      }).then(spec => {
