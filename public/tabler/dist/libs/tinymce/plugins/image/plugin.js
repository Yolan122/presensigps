...
(function () {
    ...
    const makeItems = primary => {
      ...
      const imageList = primary.imageList.map(items => ({
        name: 'images',
        type: 'listbox',
        label: 'Image list',
        items
      ...
      const imageDescription = {
        ...
        enabled: !(primary.hasAccessibilityOptions && primary.image.isDecorative)
      };
      const imageTitle = {
        name: 'title',
        type: 'input',
      ...
      const classList = primary.classList.map(items => ({
        name: 'classes',
        type: 'listbox',
        label: 'Class',
        items
      ...
      return flatten([
        [imageUrl],
        imageList.toArray(),
        primary.hasAccessibilityOptions && primary.hasDescription ? [isDecorative] : [],
        primary.hasDescription ? [imageDescription] : [],
        primary.hasImageTitle ? [imageTitle] : [],
        primary.hasDimensions ? [imageDimensions] : [],
        [{
            ...getDialogContainerType(primary.classList.isSome() && primary.hasImageCaption),
            items: flatten([
              classList.toArray(),
              primary.hasImageCaption ? [caption] : []
            ])
          }]
      ]);
    };
    const makeTab$1 = primary => ({
      title: 'General',
      name: 'general',
      items: makeItems(primary)
    });
    const MainTab = {
      makeTab: makeTab$1,
      makeItems
    ...
    const createState = primary => ({
      prevImage: ListUtils.findEntry(primary.imageList, primary.image.src),
      prevAlt: primary.image.alt,
      open: true
    });
    ...
    const addPrependUrl2 = (primary, srcURL) => {
      if (!/^(?:[a-zA-Z]+:)?\/\//.test(srcURL)) {
        return primary.prependURL.bind(prependUrl => {
    ...
    const addPrependUrl = (primary, api) => {
      const data = api.getData();
      addPrependUrl2(primary, data.src.value).each(srcURL => {
        api.setData({
          src: {
            value: srcURL,
            meta: data.src.meta
    ...
    const formFillFromMeta2 = (primary, data, meta) => {
      if (primary.hasDescription && isString(meta.alt)) {
        data.alt = meta.alt;
      }
      if (primary.hasAccessibilityOptions) {
        data.isDecorative = meta.isDecorative || data.isDecorative || false;
      }
      if (primary.hasImageTitle && isString(meta.title)) {
        data.title = meta.title;
      }
      if (primary.hasDimensions) {
      ...
      if (isString(meta.class)) {
        ListUtils.findEntry(primary.classList, meta.class).each(entry => {
          data.classes = entry.value;
        });
      }
      if (primary.hasImageCaption) {
        if (isBoolean(meta.caption)) {
          data.caption = meta.caption;
        }
      }
      if (primary.hasAdvTab) {
    ...
    const formFillFromMeta = (primary, api) => {
      const data = api.getData();
      const meta = data.src.meta;
      if (meta !== undefined) {
        const newData = deepMerge({}, data);
        formFillFromMeta2(primary, newData, meta);
        api.setData(newData);
      }
    };
    const calculateImageSize = (helpers, primary, state, api) => {
      const data = api.getData();
      const url = data.src.value;
      const meta = data.src.meta || {};
      if (!meta.width && !meta.height && primary.hasDimensions) {
        if (isNotEmpty(url)) {
    ...
    const updateImagesDropdown = (primary, state, api) => {
      const data = api.getData();
      const image = ListUtils.findEntry(primary.imageList, data.src.value);
      state.prevImage = image;
      api.setData({ images: image.map(entry => entry.value).getOr('') });
    };
    const changeSrc = (helpers, primary, state, api) => {
      addPrependUrl(primary, api);
      formFillFromMeta(primary, api);
      calculateImageSize(helpers, primary, state, api);
      updateImagesDropdown(primary, state, api);
    };
    const changeImages = (helpers, primary, state, api) => {
      const data = api.getData();
      const image = ListUtils.findEntry(primary.imageList, data.images);
      image.each(img => {
      ...
      state.prevImage = image;
      changeSrc(helpers, primary, state, api);
    };
    const changeFileInput = (helpers, primary, state, api) => {
      const data = api.getData();
      ...
      head(data.fileinput).fold(() => {
        ...
        blobToDataUri(file).then(dataUrl => {
          const blobprimary = helpers.createBlobCache(file, blobUri, dataUrl);
          if (primary.automaticUploads) {
            helpers.uploadImage(blobprimary).then(result => {
            ...
            helpers.addToBlobCache(blobprimary);
            updateSrcAndSwitchTab(blobprimary.blobUri());
            api.unblock();
          }
        });
      });
    };
    const changeHandler = (helpers, primary, state) => (api, evt) => {
      if (evt.name === 'src') {
        changeSrc(helpers, primary, state, api);
      } else if (evt.name === 'images') {
        changeImages(helpers, primary, state, api);
      } else if (evt.name === 'alt') {
        state.prevAlt = api.getData().alt;
      } else if (evt.name === 'fileinput') {
        changeFileInput(helpers, primary, state, api);
    ...
    const makeDialogBody = primary => {
      if (primary.hasAdvTab || primary.hasUploadUrl || primary.hasUploadHandler) {
        const tabPanel = {
          type: 'tabpanel',
          tabs: flatten([
            [MainTab.makeTab(primary)],
            primary.hasAdvTab ? [AdvTab.makeTab(primary)] : [],
            primary.hasUploadTab && (primary.hasUploadUrl || primary.hasUploadHandler) ? [UploadTab.makeTab(primary)] : []
          ])
        };
        return tabPanel;
      } else {
        const panel = {
          type: 'panel',
          items: MainTab.makeItems(primary)
        };
        return panel;
      }
    };
    const submitHandler = (editor, primary, helpers) => api => {
      const data = deepMerge(fromImageData(primary.image), api.getData());
      const finalData = {
        ...data,
        style: getStyleValue(helpers.normalizeCss, toImageData(data, false))
      };
      editor.execCommand('mceUpdateImage', false, toImageData(finalData, primary.hasAccessibilityOptions));
    ...
    const addToBlobCache = editor => blobprimary => {
      editor.editorUpload.blobCache.add(blobprimary);
    };
    ...
    const uploadImage = editor => blobprimary => global$1(editor).upload([blobprimary], false).then(results => {
      var _a;
      if (results.length === 0) {
    ...
    const Dialog = editor => {
      ...
      const open = () => {
        collect(editor).then(primary => {
          const state = createState(primary);
          return {
            title: 'Insert/Edit Image',
            size: 'normal',
            body: makeDialogBody(primary),
            buttons: [
              {
                type: 'cancel',
                name: 'cancel',
            ...
            initialData: fromImageData(primary.image),
            onSubmit: submitHandler(editor, primary, helpers),
            onChange: changeHandler(helpers, primary, state),
