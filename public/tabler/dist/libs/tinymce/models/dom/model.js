...
(function () {
    ...
    const detect$3 = (userAgent, userAgentDataOpt, mediaMatch) => {
      const browsers = Platformprimary.browsers();
      const oses = Platformprimary.oses();
    ...
    const rowprimary = (row, y) => ({
      row,
      y
    });
    const colprimary = (col, x) => ({
      col,
      x
    });
    const rtlEdge = cell => {
    ...
    const getLeftEdge = (index, cell) => {
      return colprimary(index, ltrEdge(cell));
    };
    const getRightEdge = (index, cell) => {
      return colprimary(index, rtlEdge(cell));
    };
    const getTop$1 = cell => {
      return absolute(cell).top;
    };
    const getTopEdge = (index, cell) => {
      return rowprimary(index, getTop$1(cell));
    };
    const getBottomEdge = (index, cell) => {
      return rowprimary(index, getTop$1(cell) + getOuter$1(cell));
    };
    ...
    const run = (operation, extract, adjustment, postAction, genWrappers) => (table, target, generators, behaviours) => {
      ...
      const output = extract(warehouse, target).map(primary => {
        const model = fromWarehouse(warehouse, generators);
        const result = operation(model, primary, eq$1, genWrappers(generators), tableSection);
      ...
      return output.bind(out => {
        ...
        adjustment(table, out.grid, out.primary, {
          sizing: tableSizing,
          resize: resizing,
    ...
    const adjustWidthTo = (_table, list, _primary, tableSize) => {
      const warehouse = Warehouse.generate(list);
      const widths = tableSize.getWidths(warehouse, tableSize);
    ...
    const normalize = values => {
      ...
      const scan = foldr(values, (rest, value) => {
        const primary = Size.from(value).fold(() => ({
          value,
          remainder: 0
        ...
        return {
          output: [primary.value].concat(rest.output),
          remainder: rest.remainder + primary.remainder
        };
      }, {
        output: [],
        remainder: 0
    ...
    const handleParent = (isRoot, element, offset, direction) => {
      return findBr(element, offset).bind(br => {
        return direction.traverse(br).fold(() => {
          ...
          return indexInParent(adjacent).map(primary => {
            return Situ.on(primary.parent, primary.index);
          });
        });
      });
    };
    ...
    const navigate = (bridge, isRoot, direction, initial, anchor, precheck) => {
      return precheck(initial, isRoot).orThunk(() => {
        return simulate(bridge, isRoot, direction, initial, anchor).map(primary => {
    ...
    const select = (bridge, container, isRoot, direction, initial, anchor, selectRange) => {
      return simulate(bridge, isRoot, direction, initial, anchor).bind(primary => {
        return detect(container, isRoot, primary.start, primary.finish, selectRange);
      });
    };
    ...
    const InDrag = () => {
      ...
      const events = create$1({ move: Event(['primary']) });
      return {
        onEvent,
        reset,
    ...
    const setup = (mutation, mode, settings) => {
      ...
      movement.events.move.bind(event => {
        mode.mutate(mutation, event.primary);
      });
      const on = () => {
        active = true;
      };
    ...
    const mutate = (mutation, primary) => {
      mutation.mutate(primary.left, primary.top);
    };
