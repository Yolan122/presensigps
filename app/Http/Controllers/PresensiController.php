...
class PresensiController extends Controller
{
    ...
    public function create($kode_jam_kerja)
    {
        ...
        $nik = Auth::guard('pengguna')->user()->nik;
        $hariini = date("Y-m-d");
    ...
    public function store(Request $request)
    {
        ...
        $nik = Auth::guard('pengguna')->user()->nik;
        $status_location = Auth::guard('pengguna')->user()->status_location;
        $hariini = date("Y-m-d");
        $jamsekarang = date("H:i");
        $tgl_sebelumnya = date('Y-m-d', strtotime("-1 days", strtotime($hariini)));
        $cekpresensi_sebelumnya = DB::table('presensi')
        ...
        $datapengguna = DB::table('pengguna')->where('nik', $nik)->first();
    ...
    public function editprofile()
    {
        ...
        return view('presensi.editprofile', compact('pengguna'));
    }
    ...
    public function monitoring()
    {
        ...
        return view('presensi.monitoring', compact('cabang', 'kategori'));
    }
    ...
    public function getpresensi(Request $request)
    {
        ...
        $query->selectRaw(
            'pengguna.nik, nama_lengkap, pengguna.kode_dept, pengguna.kode_cabang,
        ...
        $query->leftJoin(
            ...
            function ($join) {
                $join->on('pengguna.nik', '=', 'datapresensi.nik');
            }
        );
        ...
        //         ->select('presensi.*', 'nama_lengkap', 'pengguna.kode_dept', 'jam_masuk', 'nama_jam_kerja', 'jam_masuk', 'jam_pulang', 'keterangan')
        //         ->leftJoin('jam_kerja', 'presensi.kode_jam_kerja', '=', 'jam_kerja.kode_jam_kerja')
        //         ->leftJoin('pengajuan_izin', 'presensi.kode_izin', '=', 'pengajuan_izin.kode_izin')
        //         ->join('pengguna', 'presensi.nik', '=', 'pengguna.nik')
        //         ->join('kategori', 'pengguna.kode_dept', '=', 'kategori.kode_dept')
        //         ->where('tgl_presensi', $tanggal)
        //         ->where('pengguna.kode_dept', $kode_dept)
        //         ->where('pengguna.kode_cabang', $kode_cabang)
        //         ->get();
    ...
    public function laporan()
    {
        ...
        return view('presensi.laporan', compact('namabulan', 'pengguna'));
    }
    ...
    public function cetaklaporan(Request $request)
    {
        ...
        $tahun = $request->tahun;
        $namabulan = ["", "Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"];
        $pengguna = DB::table('pengguna')->where('nik', $nik)
            ->join('kategori', 'pengguna.kode_dept', '=', 'kategori.kode_dept')
            ->first();
        ...
        if (isset($_POST['exportexcel'])) {
            ...
            header("Content-Disposition: attachment; filename=Laporan Presensi Pengguna $time.xls");
            return view('presensi.cetaklaporanexcel', compact('bulan', 'tahun', 'namabulan', 'pengguna', 'presensi'));
        }
        return view('presensi.cetaklaporan', compact('bulan', 'tahun', 'namabulan', 'pengguna', 'presensi'));
    }
    ...
    public function rekap()
    {
        ...
        $cabang = DB::table('cabang')->orderBy('kode_cabang')->get();
        return view('presensi.rekap', compact('namabulan', 'kategori', 'cabang'));
    }
    ...
    public function cetakrekap(Request $request)
    {
        ...
        $namabulan = ["", "Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"];
        $datalibur = getpenggunalibur($dari, $sampai);
        ...
        $query = Pengguna::query();
        $query->selectRaw(
            "$field_date pengguna.nik, nama_lengkap, jabatan"
        );
        ...
        $query->leftJoin(
            ...
            function ($join) {
                $join->on('pengguna.nik', '=', 'presensi.nik');
            }
        );
        ...
        if (isset($_POST['exportexcel'])) {
            ...
            header("Content-type: application/vnd-ms-excel");
            // Mendefinisikan nama file ekspor "hasil-export.xls"
            header("Content-Disposition: attachment; filename=Rekap Presensi Pengguna $time.xls");
        }
    ...
    public function izinsakit(Request $request)
    {
        ...
        $query->select(
            ...
            'status_approved',
            'keterangan',
            'pengguna.kode_cabang',
            'pengguna.kode_dept',
            'doc_sid'
        );
        $query->join('pengguna', 'pengajuan_izin.nik', '=', 'pengguna.nik');
        ...
        $cabang = DB::table('cabang')->orderBy('kode_cabang')->get();
        $kategori = DB::table('kategori')->orderBy('kode_dept')->get();
        return view('presensi.izinsakit', compact('izinsakit', 'cabang', 'kategori'));
    }
    ...
    public function koreksipresensi(Request $request)
    {
        ...
        return view('presensi.koreksipresensi', compact('pengguna', 'tanggal', 'jamkerja', 'presensi'));
    }
